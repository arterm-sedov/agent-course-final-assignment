...
# --- Agent Evaluation Logic (main page, no log file handling) ---
def run_and_submit_all(profile: gr.OAuthProfile | None):
    ...
    # --- Save results table as CSV for download (persistency only, not for UI) ---
    try:
    ...
    try:
        ...
        # Save final status to a text file for persistency only
        try:
            score_path = f"logs/{timestamp}.score.txt"
            with open(score_path, "w", encoding="utf-8") as f:
                f.write(final_status)
        except Exception as e:
            print(f"⚠️ Failed to save score file: {e}")
        return final_status, results_df
    except Exception as e:
        status_message = f"Submission Failed: {e}"
        print(status_message)
        # Save error status to a text file for persistency only
        try:
            score_path = f"logs/{timestamp}.score.txt"
            with open(score_path, "w", encoding="utf-8") as f:
...
def list_logs():
    log_dir = "logs"
    allowed_exts = {'.csv', '.log', '.png', '.jpg', '.txt', '.md', '.yaml', '.yml'}
